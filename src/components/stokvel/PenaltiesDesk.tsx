...
export default function PenaltiesDesk({
  ...
  const [penaltyType, setPenaltyType] = useState<string>('late_payment');
  ...
  const handleExecuteWaiver = async () => {
    ...
    try {
      const res = await fetch(`/api/stokvel/penalties/${waivingPenaltyId}/waive`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
            ...
            <form onSubmit={handleApplyPenalty} className="space-y-3 text-xs">
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  className="w-full p-2.5 bg-stone-50 border border-stone-300 rounded-xl font-bold"
                >
                  <option value="late_payment">Late Weekly Payment (30% fine)</option>
                  <option value="withdrawal">Voluntary Early Withdrawal (30% forfeiture)</option>
                  <option value="failed_inspection">Vehicle Inspection Failure (R2,500 fine)</option>
                  <option value="late_insurance">Late Insurance Premium (R1,000 fine)</option>
                  <option value="missed_meeting">Missed Compulsory Assembly Meeting (R500 fine)</option>
                </select>
              </div>
