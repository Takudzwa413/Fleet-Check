...
export default function VehicleBenefitPipeline({
  ...
  const handleSaveBenefitRecord = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/stokvel/projects/${project.id}/benefits`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          tracker_device_ref: trackerDeviceRef,
          insurance_policy_number: `SAN-POL-${Math.floor(100000 + Math.random() * 900000)}`,
  ...
  const handleExecuteHandover = async () => {
    ...
    try {
      const res = await fetch(`/api/stokvel/benefits/${completingBenefitId}/handover`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
