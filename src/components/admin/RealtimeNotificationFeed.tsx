...
interface RealtimeNotificationFeedProps {
  token: string;
  onSelectTab: (tab: string, itemId?: string, title?: string) => void;
}
...
export default function RealtimeNotificationFeed({ token, onSelectTab }: RealtimeNotificationFeedProps) {
  ...
  const handleNotificationClick = (notif: AdminNotification) => {
    ...
    setIsOpen(false);
    setActiveToast(null);
    let targetTab = notif.link_tab || 'complaints';
    if (targetTab === 'incidents' || targetTab === 'complaint') targetTab = 'complaints';
    if (targetTab === 'pending_verifications' || targetTab === 'verification') targetTab = 'verifications';
    if (targetTab === 'dispute') targetTab = 'disputes';
    onSelectTab(targetTab, notif.item_id, notif.title);
  };
