...
export default function AdminAccountSettingsModal({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setPhone(user.phone || '');
      setTwoFactorEnabled(Boolean(user.security_settings?.two_factor_enabled));
      setEmailAlertsCritical(user.security_settings?.email_alerts_critical ?? true);
      setLoginAlerts(user.security_settings?.login_alerts ?? true);
      setSessionTimeoutMins(String(user.security_settings?.session_timeout_mins ?? 60));
  ...
  const handleSecuritySave = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/admin/profile', {
        ...
        body: JSON.stringify({
          ...
          security_settings: {
            ...
            email_alerts_critical: emailAlertsCritical,
            login_alerts: loginAlerts,
            session_timeout_mins: Number(sessionTimeoutMins) || 60
          }
        })
      });
      ...
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.error || 'Failed to save security preferences');
      }
      ...
      setSecurityError(err.message || 'Error saving security preferences');
    } finally {
