...
import { Users, Plus, Edit3, Trash2, Building2, Search, RefreshCw, Phone, Mail, MapPin, ShieldCheck, ArrowRightLeft, FileCheck } from 'lucide-react';
import DriverDocumentsReviewModal from './DriverDocumentsReviewModal';
...
export default function DriversManagement({ token, initialSearchQuery = '' }: DriversManagementProps) {
  ...
  // Documents Review Modal State
  const [reviewingDocsDriver, setReviewingDocsDriver] = useState<any | null>(null);
  ...
  return (
    <div className="space-y-6">
          ...
          {filtered.map((driver) => (
            <div key={driver.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4 hover:border-slate-300 transition-all flex flex-col justify-between">
              ...
              <div className="flex items-center space-x-2 pt-3 border-t border-slate-100">
                ...
                <button
                  onClick={() => setReviewingDocsDriver(driver)}
                  className="py-1.5 px-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-lg transition-colors cursor-pointer"
      ...
      {reviewingDocsDriver && (
        <DriverDocumentsReviewModal
          driverId={reviewingDocsDriver.id}
          driverName={`${reviewingDocsDriver.first_name} ${reviewingDocsDriver.surname}`}
          token={token}
          onClose={() => setReviewingDocsDriver(null)}
        />
      )}
