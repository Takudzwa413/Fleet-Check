...
import { Users, Plus, Edit3, Trash2, ShieldCheck, Phone, Mail, MapPin, Search, AlertTriangle, CheckCircle, RefreshCw, HelpCircle, UserCheck } from 'lucide-react';
...
export default function MyFleetDriversModule({ token }: MyFleetDriversModuleProps) {
  ...
  const [verifyingDriver, setVerifyingDriver] = useState<any | null>(null);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button
          ...
          className="w-full sm:w-auto px-4 py-2 bg-[#1f1f1f] hover:bg-stone-800 text-white font-bold text-xs rounded-xl shadow-xs transition-colors flex items-center justify-center space-x-2 cursor-pointer min-h-[44px] shrink-0"
        >
            ...
            <div key={driver.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-4 hover:border-slate-300 transition-all flex flex-col justify-between">
              ...
              <div className="flex items-center space-x-2 pt-3 border-t border-slate-100">
                <button
                  ...
                  className="flex-1 py-1.5 bg-stone-900 hover:bg-stone-800 text-white font-bold text-xs rounded-lg transition-colors flex items-center justify-center space-x-1 cursor-pointer"
                  title="Upload or request verification documentation"
                >
                ...
                <button
                  ...
                  className="py-1.5 px-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-lg transition-colors flex items-center justify-center space-x-1 cursor-pointer"
                >
                ...
                <button
                  ...
                  className="py-1.5 px-2.5 bg-red-50 hover:bg-red-100 text-red-700 font-bold text-xs rounded-lg transition-colors flex items-center space-x-1 cursor-pointer"
      ...
      {verifyingDriver && (
        <DriverVerificationModal
          driverId={verifyingDriver.id || verifyingDriver.driver_id}
          driverName={`${verifyingDriver.first_name} ${verifyingDriver.surname}`}
          token={token}
