          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {driverRequests.map((req) => (
              <div key={req.id} className="p-5 bg-slate-50 rounded-xl border border-slate-200 space-y-4 shadow-xs">
                ...
                {req.status === 'pending' && (
                  <div className="pt-3 border-t border-slate-200/80 flex flex-col sm:flex-row justify-end gap-2">
                    <button
                      type="button"
                      disabled={respondingReqId === req.id}
                      onClick={() => handleRespondDriverRequest(req.id, 'reject')}
                      className="w-full sm:w-auto px-3.5 py-2 bg-red-50 hover:bg-red-100 text-red-700 border border-red-200 text-xs font-semibold rounded-lg transition-colors cursor-pointer min-h-[40px] text-center"
                    >
                      Decline Request
                    </button>
                    <button
                      ...
                      className="w-full sm:w-auto px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold rounded-lg shadow-sm transition-colors cursor-pointer flex items-center justify-center space-x-1 min-h-[40px]"
                    >
