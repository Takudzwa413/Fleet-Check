...
import {
  ...
  Sparkles, CheckCircle2, Award, ChevronRight, HelpCircle, Info, Car
} from 'lucide-react';
...
export default function FleetOwnerDashboard({
  ...
  const [dbTab, setDbTab] = useState<'search' | 'my_drivers' | 'submit_complaint' | 'my_complaints' | 'verification_docs' | 'driver_requests' | 'vehicle_listings'>('search');
  ...
  const navItems: NavItem[] = [
    {
      ...
      tooltip: 'Centralized cross-platform verification gateway to search drivers by name, phone, SA ID, license, or plate number.'
    },
    {
      id: 'my_drivers',
    ...
    },
    {
      ...
      category: 'FLEET OPS',
      tooltipTitle: 'My Vehicle Listings',
      tooltip: 'Advertise vehicles available for hire on the public Vehicle Marketplace, pending admin review.'
    },
    {
  ...
  return (
    <DashboardLayout
      ...
      statChips={statChips}
      showRightPanel={dbTab === 'my_drivers' || dbTab === 'verification_docs'}
    ...
    >
      ...
      {dbTab === 'vehicle_listings' && (
        <VehicleListingsModule token={token} />
      )}
