...
export default function DriverDashboard({ user, onLogout, onOpenChat, onOpenChatThreads }: DriverDashboardProps) {
  ...
  const navItems: NavItem[] = [
    ...
    },
    {
      ...
      category: 'OVERVIEW',
      tooltipTitle: 'Driver Search Gateway',
      tooltip: 'Access the centralized Driver Search Gateway to query driver registry records, risk scores, and credential clearances.'
    },
    {
      id: 'fleet_link',
    ...
    {
      id: 'reviews',
      label: 'Fleet Owner Reviews',
      icon: <Star className="h-4 w-4" />,
      category: 'REPUTATION',
      tooltipTitle: 'Fleet Owner Reviews & Ratings',
      tooltip: 'Inspect star ratings and feedback left by verified fleet management partners.'
    },
    {
      id: 'documents',
      label: 'My Documents',
      ...
      category: 'ACCOUNT',
      tooltipTitle: 'Verification Documents',
      tooltip: "Upload your driver's licence, passport, platform profile screenshot, and safety-check documents for admin review."
    }
  ];
  ...
  return (
    <DashboardLayout
      ...
      searchPlaceholder="Search drivers, fleet owners, companies, or license registry..."
      onSearch={(q) => {
        setActiveSearchQuery(q);
        if (q.trim() && activeTab !== 'search_gateway') {
          setActiveTab('search_gateway');
        }
      }}
      ...
      statChips={statChips}
      showRightPanel={activeTab === 'profile'}
    ...
    >
      ...
      {/* TAB 2: Driver Search Gateway */}
      {activeTab === 'search_gateway' && (
        <div className="space-y-6">
          <DriverSearchModule
            isVerified={true}
            token={authToken}
            getRiskColorBadge={(level: string) => {
              switch (level) {
          ...
          {/* 30-Day Search Frequency and Trending Intelligence Chart */}
                  ...
                  {onOpenChat && (
                    <div className="flex justify-end">
                      <button
                        ...
                        className="px-3.5 py-1.5 border border-stone-200 hover:bg-stone-100 text-stone-700 text-xs font-bold rounded-xl flex items-center space-x-1.5 transition-colors cursor-pointer"
                      >
                        <Send className="h-3 w-3 text-stone-700" />
                        <span>Message Fleet Owner / Admin</span>
                      </button>
                    </div>
                  )}
      ...
      {/* TAB 4: Performance Reviews from Fleet Owners */}
      {activeTab === 'reviews' && (
        <div className="space-y-6">
          <DriverReviewsSection
            driverId={profile?.id || user.id}
            driverName={user.name}
            isVerifiedFleetOwner={false}
            token={authToken}
          />
        </div>
      )}
      ...
      {activeTab === 'documents' && (
        profile?.id ? (
          <DriverDocumentsModule token={authToken} driverProfileId={profile.id} />
        ) : (
          <div className="bg-white p-8 text-center rounded-2xl border border-stone-200 shadow-2xs space-y-2">
            <RefreshCw className="h-6 w-6 text-stone-300 mx-auto animate-spin" />
            <p className="text-stone-500 text-xs">
              {loading ? 'Loading your driver profile...' : 'Your driver profile could not be loaded. Please refresh the page to upload verification documents.'}
            </p>
          </div>
        )
      )}
