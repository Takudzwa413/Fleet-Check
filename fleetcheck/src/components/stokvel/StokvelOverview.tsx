    ...
    <div className="space-y-6">
      ...
      {userSlot && (
        <div className="bg-white border border-stone-200 rounded-2xl p-5 shadow-xs">
          ...
          {userSlot.status === 'default_risk' && userSlot.default_grace_period_end_date && (
            <div className="mt-3 p-3 bg-orange-50 border border-orange-200 rounded-xl text-xs text-orange-800 font-semibold">
              Your outstanding balance exceeds the default threshold. Please settle it by{' '}
              <strong>{userSlot.default_grace_period_end_date}</strong> to avoid escalation for dismissal review.
            </div>
          )}
        </div>
      )}
            ...
            (() => {
              ...
              return (
                <div className="bg-emerald-50/60 border border-emerald-200/80 rounded-xl p-4 space-y-3">
                  {/* currentBeneficiary may be a different member than the viewer, whose slot
                      the API sanitizes for privacy (financial fields undefined) unless the
                      viewer is privileged or it happens to be their own slot. */}
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-extrabold text-emerald-900">
                      {typeof currentBeneficiary.payout_amount === 'number'
                  ...
                  <div>
                    ...
                    <p className="text-xs text-stone-600">
                      {currentBeneficiary.slot_type_name}
                      {typeof currentBeneficiary.deposit_paid === 'number' ? ` • Deposit Paid: R ${currentBeneficiary.deposit_paid.toLocaleString()}` : ''}
                    </p>
                  </div>
          ...
          {slots.map((s, idx) => (
            ...
            >
              ...
              <div className="mt-2 text-xs space-y-1 text-stone-600">
                {/* Non-privileged viewers only receive full financial figures (payout_amount,
                    weekly_contribution, outstanding_amount) for their OWN slot - the API
                    sanitizes every other member's slot for privacy (see GET .../slots in
                    projectRoutes.ts). Fields are therefore undefined here for other members'
                    cards, so we show a privacy-safe placeholder instead of crashing. */}
                <div className="flex justify-between">
                  <span>Target:</span>
                  <span className="font-bold text-stone-900">
                    {typeof s.payout_amount === 'number' ? `R ${s.payout_amount.toLocaleString()}` : 'Private'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Weekly:</span>
                  <span className="font-bold text-stone-900">
                    {typeof (s.current_weekly_contribution ?? s.weekly_contribution) === 'number'
                ...
                <div className="flex justify-between">
                  <span>Status:</span>
                  <span className={`font-bold capitalize ${typeof s.outstanding_amount === 'number' && s.outstanding_amount > 0 ? 'text-red-600' : 'text-emerald-700'}`}>
                    {s.status.replace('_', ' ')}
                  </span>
                </div>
              </div>
