              ...
              {sortedSlots.map(slot => (
                <tr key={slot.id} className="hover:bg-stone-50/70 transition-colors">
                  ...
                  {/* Non-privileged viewers only receive full financial figures for their OWN
                      slot - the API sanitizes every other member's slot for privacy (see
                      GET .../slots in projectRoutes.ts), so those fields are undefined here.
                      Show a privacy-safe placeholder instead of crashing on the missing data. */}
                  <td className="px-4 py-3.5 text-right font-bold text-stone-900">
                    {typeof (slot.current_weekly_contribution ?? slot.weekly_contribution) === 'number'
                      ? `R ${(slot.current_weekly_contribution ?? slot.weekly_contribution)!.toLocaleString()}`
                      : <span className="text-stone-400 font-normal italic">Private</span>}
                  </td>
                  <td className="px-4 py-3.5 text-right">
                    {typeof slot.deposit_paid === 'number' && typeof slot.deposit_required === 'number' ? (
                      <>
