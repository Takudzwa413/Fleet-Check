...
import {
  ...
  Search, Check, X, Link, ArrowUpRight, Car
} from 'lucide-react';
...
export default function DriverDashboard({ user, onLogout }: DriverDashboardProps) {
  const [activeTab, setActiveTab] = useState<'profile' | 'fleet_link' | 'complaints' | 'stokvel'>('profile');
  const stokvelToken = localStorage.getItem('fc_token') || '';
  ...
  const navItems: NavItem[] = [
    ...
    {
      id: 'stokvel',
      label: 'My Vehicle Acquisition Project',
      icon: <Car className="h-4 w-4" />,
      category: 'OPERATIONS',
      tooltipTitle: 'Vehicle Acquisition Stokvel Project',
      tooltip: 'Track your slot, weekly contributions, proof of payment uploads, penalties, and vehicle acquisition progress.'
    }
  ];
  ...
  return (
    ...
    >
      ...
      {/* TAB 4: My Vehicle Acquisition Stokvel Project */}
      {activeTab === 'stokvel' && (
        <StokvelDashboard
          user={user}
          token={stokvelToken}
