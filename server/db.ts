...
// Awaited variant of safeSetDoc, for the rare write where the caller needs to
// know the data actually reached Firestore before responding to the client
// (e.g. a freshly-issued session token) rather than the usual fire-and-forget
// pattern used everywhere else in this file.
export async function awaitedSetDoc(col: string, id: string, data: any, options?: { merge?: boolean }): Promise<void> {
  if (!id || !col) return;
  const cleanData = sanitizeForFirestore(data);
  const docRef = firestoreDb.collection(col).doc(id);
  await (options?.merge ? docRef.set(cleanData, { merge: true }) : docRef.set(cleanData));
}
...
// Persisted in Firestore (not an in-memory Map) so login sessions and reset
// tokens survive server restarts/redeploys and stay consistent if this ever
...
export class LocalDatabase {
  ...
  private async initFirebase() {
    try {
      ...
      const collections = [
        ...
        { name: 'resetTokens', key: 'resetTokens' }
      ];
  ...
  // Sessions and password-reset tokens: persisted in Firestore (via the same
  // live-sync mechanism as every other collection here) rather than an
  // in-memory Map, so a login survives a server restart/redeploy and stays
  // valid if this process is ever scaled to more than one instance.
  ...
  // Awaited (unlike every other add* method here) so a login response is
  // only sent once the session has actually reached Firestore -- otherwise a
  // restart in the small window before the fire-and-forget write completed
  // would silently drop a token the client already believes is valid.
  public async addSession(session: SessionRecord): Promise<void> {
    if (!this.data.sessions) this.data.sessions = [];
    this.data.sessions.push(session);
    try {
      await awaitedSetDoc('sessions', session.token, session);
    } catch (err: any) {
      console.error(`[Firestore Sync] Failed to persist session ${session.token}:`, err?.message || err);
    }
  }
  ...
  public deleteSession(token: string) {
    ...
    safeDeleteDoc('sessions', token);
  }

  public deleteAllSessionsForUser(userId: string) {
    if (!this.data.sessions) this.data.sessions = [];
    const toDelete = this.data.sessions.filter(s => s.user_id === userId);
    this.data.sessions = this.data.sessions.filter(s => s.user_id !== userId);
    toDelete.forEach(s => safeDeleteDoc('sessions', s.token));
  }

  public getResetToken(token: string): ResetTokenRecord | undefined {
    const rec = (this.data.resetTokens || []).find(r => r.token === token);
  ...
  public addResetToken(rec: ResetTokenRecord) {
    if (!this.data.resetTokens) this.data.resetTokens = [];
    this.data.resetTokens.push(rec);
    safeSetDoc('resetTokens', rec.token, rec);
  }

  public deleteResetToken(token: string) {
    if (!this.data.resetTokens) this.data.resetTokens = [];
    this.data.resetTokens = this.data.resetTokens.filter(r => r.token !== token);
    safeDeleteDoc('resetTokens', token);
  }

  // Periodic sweep so expired sessions/tokens that are never looked up again
  // (abandoned reset links, sessions nobody logged out of) don't accumulate
  // in Firestore forever.
  public cleanupExpired() {
    const now = Date.now();
    const expiredSessions = (this.data.sessions || []).filter(s => new Date(s.expires_at).getTime() < now);
    expiredSessions.forEach(s => this.deleteSession(s.token));
    const expiredTokens = (this.data.resetTokens || []).filter(r => new Date(r.expires_at).getTime() < now);
    expiredTokens.forEach(r => this.deleteResetToken(r.token));
  }
  ...
  public deleteRiskScoresForDriver(driverId: string) {
    const toDelete = this.data.riskScores.filter(r => r.driver_id === driverId);
    this.data.riskScores = this.data.riskScores.filter(r => r.driver_id !== driverId);
    toDelete.forEach(r => safeDeleteDoc('riskScores', r.id));
  }
  ...
  public deleteDriverDocument(id: string) {
    if (!this.data.driverDocuments) this.data.driverDocuments = [];
    this.data.driverDocuments = this.data.driverDocuments.filter(d => d.id !== id);
    safeDeleteDoc('driverDocuments', id);
  }

  public deleteDocument(id: string) {
    this.data.fleetOwnerDocuments = this.data.fleetOwnerDocuments.filter(d => d.id !== id);
    safeDeleteDoc('documents', id);
  }
