...
export class ProjectDatabase {
  ...
  public createProject(projectInput: Partial<Project>, actorUser: User): Project {
    ...
    const newProject: Project = {
      ...
      constitution_document_name: projectInput.constitution_document_name,
      constitution_document_data: projectInput.constitution_document_data,
  ...
  public addProjectMember(memberInput: Partial<ProjectMember>, actorUser: User): ProjectMember {
    ...
    const newMember: ProjectMember = {
      ...
      user_email: memberInput.user_email,
      user_phone: memberInput.user_phone,
  ...
  public uploadPOP(paymentInput: Partial<ProjectPayment>, actorUser: User): ProjectPayment {
    ...
    const newPayment: ProjectPayment = {
      ...
      project_member_id: paymentInput.project_member_id,
      slot_id: paymentInput.slot_id,
      user_id: actorUser.id,
      user_name: actorUser.name,
      ...
      file_name: paymentInput.file_name,
      file_data: paymentInput.file_data,
      file_type: paymentInput.file_type,
  ...
  public recalculateSlotBalances(slotId: string) {
    ...
    // Check Arrears & Default Threshold logic
  ...
  private handleBenefitReceivedTrigger(benefit: ProjectVehicleBenefit, actorUser: User) {
    const slot = this.data.projectMemberSlots.find(s => s.id === benefit.slot_id);
    if (!slot) return;

    const now = new Date();
    const graceEnd = new Date(now);
    graceEnd.setDate(now.getDate() + 7); // 1-week grace period
    ...
    this.sendProjectNotification(
      ...
      `Congratulations! Vehicle handover is complete. A 1-week grace period has begun. Starting next cycle, your weekly contribution updates to R${newWeekly.toLocaleString()}.`,
      'success',
      slot.project_id
    );
  }
  ...
  public applyPenalty(penaltyInput: Partial<ProjectPenalty>, actorUser: User): ProjectPenalty {
    ...
    const newPenalty: ProjectPenalty = {
      ...
      linked_week_number: penaltyInput.linked_week_number,
  ...
  public saveInsuranceRecord(record: Partial<ProjectInsuranceRecord>, actorUser: User): ProjectInsuranceRecord {
    ...
    const newRecord: ProjectInsuranceRecord = {
      ...
      vehicle_benefit_id: record.vehicle_benefit_id,
  ...
  public logProjectAudit(logInput: Partial<ProjectAuditLog>) {
    ...
    const audit: ProjectAuditLog = {
      ...
      project_id: logInput.project_id,
  ...
  private async seedActionPackProject() {
    ...
    await setDoc(doc(firestoreDb, 'projects', projId), actionPackProject);
    ...
    await setDoc(doc(firestoreDb, 'projectSlotTypes', fullSlot.id), fullSlot);
    await setDoc(doc(firestoreDb, 'projectSlotTypes', halfSlot.id), halfSlot);
    ...
    for (const m of membersData) {
      ...
      await setDoc(doc(firestoreDb, 'projectMembers', pMember.id), pMember);
      ...
      await setDoc(doc(firestoreDb, 'projectMemberSlots', slotId), pSlot);
      ...
      for (let w = 1; w <= totalWeeks; w++) {
        ...
        await setDoc(doc(firestoreDb, 'projectContributionSchedules', schedId), schedule);
      }
      ...
      await setDoc(doc(firestoreDb, 'projectPayments', depositPayment.id), depositPayment);
      ...
      await setDoc(doc(firestoreDb, 'projectPayments', weekPayment.id), weekPayment);
    }
    ...
    await setDoc(doc(firestoreDb, 'projectGroupSummaries', initialSummary.id), initialSummary);
    ...
    await setDoc(doc(firestoreDb, 'projectMeetings', sampleMeeting.id), sampleMeeting);
